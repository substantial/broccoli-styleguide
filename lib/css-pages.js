...
CSSPages.prototype.extensions = defaults.extensions;
CSSPages.prototype.targetExtension = defaults.targetExtension;
...
CSSPages.prototype.processString = function(string) {
  ...
  return op.extractDocFromCSS(string)
    .then(function(cssDoc) {
      if (cssDoc === null) {
        return { metadata: 'category: "undocumented"', markdown: '' }
      } else {
        return op.splitDocument(cssDoc); // returns regexp match array
      }
    })
    .then(function(parts){
      var document = {
      ...
      // handle array from splitDocument's regexp match
      if (parts && _.isArray(parts)) {
        document.metadata = parts[1];
        document.markdown = parts[2];
      // handle properties passed
      } else if (parts && parts.metadata) {
        document.metadata = parts.metadata;
        document.markdown = parts.markdown;
      }
      return document;
    })
