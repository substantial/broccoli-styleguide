...
function extractDocFromCSS(string) {
  return new RSVP.Promise(function(resolve, reject){
    ...
    if (startIndex < 0) {
      reject('Could not extract doc from CSS; starting comment `'+startingComment+'` not found.');
      return;
    }
    var endIndex = string.indexOf(endingComment);
    if (endIndex < 0) {
      reject('Could not extract doc from CSS; ending comment `'+endingComment+'` not found.');
      return;
    }
